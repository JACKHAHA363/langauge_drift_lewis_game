    ...
    try:
        for step in range(args.steps):
            ...
            if step % args.log_steps == 0:
                ...
                # Record stats for speaker, listener, and commmunication
                stats = {'temp': temperature}
                s_stats, s_conf_mat = eval_speaker_loop(game.get_generator(1000), speaker)
                stats.update(s_stats)
                l_stats, l_conf_mat = eval_listener_loop(game.get_generator(1000), listener)
                stats.update(l_stats)
                comm_stats, l_conf_mat_gr_msg = eval_comm_loop(game.get_generator(1000), listener, speaker)
